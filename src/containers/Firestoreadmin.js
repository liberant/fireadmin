...
class Firestoreadmin extends Component {
  ...
  * @param {String} byGivenSubLink force link to field
  * @param {Function} callback function after action
  */
  updatePartOfObject(key,value,dorefresh=false,type=null,firebasePath,byGivenSubLink=null,callback=null){
    var subLink=this.state.theSubLink;
    if(byGivenSubLink!=null){
      subLink=byGivenSubLink;
    }
  ...
  */
      ...
      return (
          <div className="col-md-12" key={name}>
              <div className="card">
                  ...
                  <a  onClick={()=>{this.addDocumentToCollection(name)}}><div id="addDiv" className="card-header card-header-icon" data-background-color="purple" style={{float:"right"}}>
  ...
  generateBreadCrumb(){
      ...
      return (<div>{items.map((item,index)=>{
        ...
        return (<Link className="navbar-brand" to={path}>{item} <span className="breadcrumbSeparator">{index==items.length-1?"":"/"}</span><div className="ripple-container"></div></Link>)})}</div>)
  }
